...
func SendVerifiedEmail(to_email string, first_name string, second_name string, token string) error {
	...
	msg.SetBody("text/html", fmt.Sprintf("Приветствуем, %s %s! <br/> Для подтверждения электронной почты, пройдите, пожалуйста, по ссылке: <br/>  http://185.225.34.197/auth?verification_email_token=%s <br/> Если Вы получили это письмо по ошибке, просто игнорируйте его. <br/> Ссылка активна в течении 7 дней.", first_name, second_name, token))
